...
namespace Plugins.VFX.Volumes
{
	...
	public class SupportedOnCustomProfileAttribute : Attribute
	{
		...
		public SupportedOnCustomProfileAttribute(Type renderPipeline)
			: this(new Type[1] { renderPipeline })
		{
		}

		public SupportedOnCustomProfileAttribute(params Type[] profiles)
		{
			if (profiles == null)
			{
		...
		{
			SupportedOnCustomProfileAttribute customAttribute = type.GetCustomAttribute<SupportedOnCustomProfileAttribute>();
			return customAttribute == null || (uint)customAttribute.GetSupportedMode(renderPipelineAssetType) > 0U;
		}

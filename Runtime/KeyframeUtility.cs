...
namespace Plugins.VFX.Volumes
{
	...
	public class KeyframeUtility
	{
		...
		{
			var lhsKey = FetchKeyFromIndexClampEdge(keys, lhsIndex, startTime, endTime);
			var rhsKey = FetchKeyFromIndexClampEdge(keys, rhsIndex, startTime, endTime);

			float currValue;
			float currDeriv;
			EvalCurveSegmentAndDeriv(out currValue, out currDeriv, lhsKey, rhsKey, currTime);
...
#else
				lhsAndResultCurve.keys = EMPTY_KEYS;
				Keyframe[] keys = rhsCurve.keys;
				foreach (Keyframe keyframe in keys)
				{
					lhsAndResultCurve.AddKey(keyframe);
				}
#endif
			}
			else
				...
				while (lhsKeyCurr < lhsCurveKeys.Length || rhsKeyCurr < rhsCurveKeys.Length)
				{
					...
					if (lhsValid && rhsValid)
					{
						...
						else if (lhsKey.time < rhsKey.time)
						{
							...
							rhsKey = EvalKeyAtTime(rhsCurveKeys, rhsKeyCurr - 1, rhsKeyCurr, startTime, endTime, lhsKey.time);
							lhsKeyCurr++;
						}
						else
						{
							...
							lhsKey = EvalKeyAtTime(lhsCurveKeys, lhsKeyCurr - 1, lhsKeyCurr, startTime, endTime, rhsKey.time);
							rhsKeyCurr++;
						}
					}
					else if (lhsValid)
					...
					var dstKey = LerpSingleKeyframe(lhsKey, rhsKey, t);
